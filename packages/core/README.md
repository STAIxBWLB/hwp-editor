...
| `EditOp`, `opsToJson`, `opsToArgv`, `argvToOps`, `OP_FLAGS` | union + functions | Type-safe mirror of every `hwp edit` op, pinned to the hwp-cli contract. `opsToJson` serializes an op list to the published `edit-ops-v1` JSON the CLI engine passes to `hwp edit --ops`; `opsToArgv`/`argvToOps` are the separator-delimited argv form the tauri host still crosses with, so a queue survives a round trip. |
| `parseCatEnvelope`, `offsetToRef`, `segmentAtRef`, `segmentText` | functions | Pure helpers over the `hwp cat --with-segments --segments v2` envelope. They bridge rendered markdown offsets and the stable `{section, para}` source coordinates `EditOp` targets. |
